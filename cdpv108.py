#!/usr/bin/env python3
"""
Chrome Session Manager - Production v6
Fixed: Proper display detection with environment
"""
...
# ============================================================================
# Display Manager with VNC Detection
# ============================================================================
...
class DisplayManager:
    ...
    def _check_display_works(self, display: str) -> bool:
        """Check if display is actually working using xdpyinfo"""
        if not self._check_x_socket(display):
            logger.debug(f"No X socket for {display}")
            return False
        
        try:
            # Set DISPLAY environment for the subprocess
            env = os.environ.copy()
            env['DISPLAY'] = display
            ...
            if result.returncode == 0:
                logger.debug(f"xdpyinfo succeeded for {display}")
                return True
            else:
                logger.debug(f"xdpyinfo failed for {display}: {result.stderr[:100]}")
                return False
        except Exception as e:
            logger.debug(f"xdpyinfo error for {display}: {e}")
            return False
    ...
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        ...
        if env_display:
            ...
            if self._check_x_socket(env_display):
                if self._check_display_works(env_display):
        ...
        for session in vnc_sessions:
            ...
            if self._check_x_socket(display):
                logger.info(f"X socket found for {display}")
                
                # Try xdpyinfo with DISPLAY set
                if self._check_display_works(display):
                    ...
                    logger.warning(f"xdpyinfo failed for {display}, but socket exists. This might be a stale X socket.")
...
class ChromeLauncher:
    ...
    def build_command(self, session: Dict, use_display: bool) -> List[str]:
        ...
        if use_display:
            # Start with the URL directly so Chrome opens in VNC
    ...
    def _launch_once(self, session: Dict) -> Tuple[bool, Optional[int], str]:
        ...
        if display:
            ...
            if self.display_manager._check_display_works(display):
                use_display = True
                logger.info(f"Using display: {display}")
                console.print(f"[green]🖥️ Using display: {display}[/green]")
            else:
                logger.warning(f"Display {display} not working, falling back to headless")
                console.print(f"[yellow]⚠️ Display {display} not working, using headless[/yellow]")
                ...
                if log_file.exists():
                    with open(log_file, 'r') as f:
                        ...
                        if "cannot open display" in error_msg.lower():
                            ...
                            console.print("[yellow]💡 Try: export DISPLAY=:1 and restart VNC[/yellow]")
