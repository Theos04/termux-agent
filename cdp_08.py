#!/usr/bin/env python3
"""
Enhanced Chrome Session Manager - Full Fix Edition
With aggressive Chrome process management
"""
...
class ChromeSessionManager:
    ...
    def _kill_all_chrome_processes(self):
        ...
        if killed:
            console.print(f"[dim]Killed {len(killed)} Chrome processes[/dim]")
            time.sleep(2)
        
        return killed

    def restart_session_with_fixed_flags(self, session_id: int):
        """Force restart a session with proper Chrome flags"""
        session = self.db.get_session(session_id)
        if not session:
            console.print(f"[red]Session not found[/red]")
            return
        
        console.print(f"[yellow]🔥 Force restarting session '{session['name']}' with fixed flags...[/yellow]")
        
        # Step 1: Kill ALL Chrome processes
        console.print("[dim]Killing all Chrome processes...[/dim]")
        ...
        if os.path.exists(profile_dir):
            try:
                ...
                for root, dirs, files in os.walk(profile_dir):
                    for f in files:
                        if f.endswith('.lock') or f == 'SingletonLock' or f == 'SingletonSocket':
                            try:
        ...
        # Step 4: Start fresh with fixed flags
        console.print("[green]Starting fresh session with fixed flags...[/green]")
    ...
    def start_session(self, session_id: int, force_restart: bool = False):
        ...
        if self._is_port_in_use(session['port']):
            console.print(f"[yellow]Port {session['port']} is in use. Finding new port...[/yellow]")
        ...
        try:
            ...
            if process.poll() is None and connected:
                ...
                console.print(f"   Flags: --remote-allow-origins=*")
                ...
                console.print("[dim]Testing WebSocket connection...[/dim]")
                tabs = devtools.get_tabs()
                if tabs:
                    console.print(f"[dim]✅ {len(tabs)} tabs found - WebSocket working![/dim]")
                else:
                    console.print("[dim]No tabs found[/dim]")
                
                # Test execute a simple script
                try:
                    test_result = devtools.execute_script(tabs[0]['id'], "document.title")
                    if test_result:
                        console.print(f"[dim]✅ JavaScript execution working! Title: {test_result.get('value', 'Unknown')}[/dim]")
                except:
                    pass
            else:
...
"""
    ...
    def manage_tabs_enhanced(self, session_id: int):
        ...
        if session['status'] != 'running':
            ...
            if Confirm.ask("Start this session with fixed flags?"):
        ...
        if not devtools._ensure_connection():
            ...
            if Confirm.ask("Force restart with fixed WebSocket flags?"):
                self.restart_session_with_fixed_flags(session_id)
                time.sleep(3)
        ...
        while True:
            ...
            console.print("  [R] Force restart with fixed flags (kill all Chrome)")
            ...
            if choice == "0":
                ...
                devtools = self._get_devtools(session['port'])
                if devtools._ensure_connection():
                    console.print("[green]✅ Session restarted successfully![/green]")
                else:
                    console.print("[red]❌ Failed to restart session[/red]")
                continue
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            header = """
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Full Fix Edition    ║
...
╚══════════════════════════════════════════════════════════════╝
            ...
            menu.add_row("9", "[bold]Force Restart (Kill All)[/bold]", "Kill all Chrome and restart")
            ...
            if choice == "0":
                ...
                self.list_sessions()
                try:
                    session_id = int(Prompt.ask("Enter session ID to force restart (kills all Chrome)"))
                    self.restart_session_with_fixed_flags(session_id)
                except ValueError:
                    console.print("[red]Invalid ID[/red]")
