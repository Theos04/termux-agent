...
Uses wscat for reliable WebSocket communication
"""
...
class ChromeDevTools:
    """Chrome DevTools Protocol using wscat for WebSocket"""
    ...
    def __init__(self, host='127.0.0.1', port=9222):
        ...
        # Check for wscat
        self.wscat_path = shutil.which('wscat')
        self.websocat_path = shutil.which('websocat')
        if self.wscat_path:
            console.print("[dim]✅ Found wscat for WebSocket communication[/dim]")
        elif self.websocat_path:
            console.print("[dim]✅ Found websocat for WebSocket communication[/dim]")
        else:
            console.print("[yellow]⚠️ No WebSocket tool found. Install wscat or websocat[/yellow]")
    ...
    def execute_script(self, tab_id: str, script: str, return_by_value: bool = True) -> Optional[Dict]:
        """Execute JavaScript using wscat for WebSocket"""
        try:
            ...
            # Try wscat first
            if self.wscat_path:
                result = self._execute_with_wscat(ws_url, [enable_msg, exec_msg])
                if result:
                    return result

            # Try websocat
            if self.websocat_path:
                result = self._execute_with_websocat(ws_url, [enable_msg, exec_msg])
                if result:
                    return result

            # Fallback to Python websocket
            try:
                console.print("[dim]Using Python websocket fallback[/dim]")
                return self._execute_with_websocket(ws_url, script)
            except:
                pass

            console.print("[red]All WebSocket methods failed[/red]")
            return None
    ...
    def _execute_with_wscat(self, ws_url: str, messages: List[Dict]) -> Optional[Dict]:
        """Execute WebSocket messages using wscat"""
        try:
            # Create a temporary file for the messages
            ...
            try:
                # Use wscat with the messages file
                # wscat -c ws://url --execute "command"
                # We'll send messages one by one
                result = None
                for i, msg in enumerate(messages):
                    # For the first message (enable), we need to wait for response
                    if i == 0:
                        cmd = [
                            'wscat', '-c', ws_url,
                            '-x', json.dumps(msg)
    ...
    def _execute_with_websocat(self, ws_url: str, messages: List[Dict]) -> Optional[Dict]:
        """Execute WebSocket messages using websocat"""
        try:
            # websocat can handle multiple messages
            all_messages = '\n'.join([json.dumps(msg) for msg in messages])
            
            cmd = ['websocat', '-w', '5', ws_url]
            ...
            # Send all messages
            stdout, stderr = proc.communicate(all_messages, timeout=5)
    ...
    def _execute_with_websocket(self, ws_url: str, script: str) -> Optional[Dict]:
        """Execute script using websocket library"""
        try:
            ws = None
            try:
