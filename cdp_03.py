...
class ChromeSessionManager:
    ...
    def _find_display(self) -> Optional[str]:
        ...
        # Check for VNC server sessions using vncserver command
        try:
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    # Look for display numbers like :1, :2, etc.
                    match = re.search(r':(\d+)', line)
                    if match:
                        display_num = match.group(0)
                        # Test if we can connect to this display
                        try:
                            # Simple test - try to connect to the X server
                            sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                            sock.settimeout(1)
                            # X server typically listens on port 6000 + display number
        ...
        # Try to find VNC process directly
        try:
            result = subprocess.run(['ps', 'aux'], capture_output=True, text=True)
            for line in result.stdout.split('\n'):
                if 'vnc' in line.lower() or 'Xvnc' in line:
                    # Look for display number in the line
                    match = re.search(r':(\d+)', line)
                    if match:
                        display_num = f":{match.group(1)}"
                        console.print(f"[green]✅ Found VNC process with display: {display_num}[/green]")
                        os.environ['DISPLAY'] = display_num
                        return display_num
        except:
            pass
        ...
        for display in [':0', ':1', ':2', ':3', ':99']:
            try:
                # Try to connect to X server port
        ...
        # Try with xdpyinfo if available
        try:
            for display in [':0', ':1', ':2', ':3']:
                try:
                    ...
                    if result.returncode == 0:
                        console.print(f"[green]✅ Found X display: {display}[/green]")
                        os.environ['DISPLAY'] = display
                        return display
                except:
                    pass
    ...
    def _start_xvfb_if_needed(self) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            self.display = ':99'
            console.print("[green]✅ Started Xvfb virtual display[/green]")
            return xvfb_process
        except:
            console.print("[yellow]⚠️ Xvfb not available. Will try headless mode.[/yellow]")
            return None
    ...
    def _get_next_port(self) -> int:
        ...
        # Get all ports currently in use by sessions
        used_ports = self.db.get_all_ports()
        
        # Also check if any ports are in use by running processes
    ...
    def create_session(self):
        ...
        try:
            ...
            # Try to get another port if this one failed
    ...
    def start_session(self, session_id: int):
        ...
        try:
            ...
            with Progress(SpinnerColumn(), TextColumn("[progress.description]{task.description}"), transient=True) as progress:
                ...
                for i in range(30):  # Try for 60 seconds (30 * 2)
                    if process.poll() is not None:
                        stdout, stderr = process.communicate()
