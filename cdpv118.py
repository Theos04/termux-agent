...
Added: Session info JSON tracking with start/end/error events
"""
...
# ============================================================================
# Session Info Tracker
# ============================================================================

class SessionInfoTracker:
    """Tracks session start, end, and error events in a JSON file"""
    ...
    def log_session_start(self, session_id: int, session_data: Dict):
        ...
        with self._lock:
            ...
            logger.info(f"📝 Session start logged for {session_id}")
...
class ChromeSessionManager:
    ...
    def _start_session_internal(self, session_id: int):
        ...
        if success:
            ...
            self.session_tracker.log_session_start(session_id, session_data)
...
"""
        if recent_events:
            for event in recent_events[-5:]:
                ...
                content += f"  {icon} [{event_type}] Session {event.get('session_id')}: {event.get('session_name', '')}\n"
        else:
        ...
        try:
            ...
            if events:
                ...
                for event in events[-10:]:
                    event_type = event.get('type', 'unknown')
                    icon = "🚀" if event_type == 'start' else "🛑" if event_type == 'end' else "❌"
                    console.print(f"  {icon} {event.get('timestamp', '')} - Session {event.get('session_id')}: {event.get('session_name', '')}")
                    if event.get('error'):
