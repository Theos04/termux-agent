...
class DOMExplorer:
    ...
    def get_all_urls(self) -> List[str]:
        """Get ALL URLs from the page (both internal and external)"""
    ...
    def get_concatenated_urls(self) -> str:
        """Get all URLs concatenated with newlines"""
...
def show_all_urls(explorer: DOMExplorer):
    """Show ALL URLs found on the page"""
    console.print()
    console.print(Panel("[bold cyan]🌐 ALL URLS FOUND ON PAGE[/bold cyan]", border_style="green"))
    
    all_urls = explorer.get_all_urls()
    
    if not all_urls:
        console.print("[yellow]No URLs found on page[/yellow]")
        return
    
    # Show total count
    console.print(f"\n[bold green]Total URLs found: {len(all_urls)}[/bold green]")
    
    # Show URLs in a table
    table = Table(title=f"URLs ({len(all_urls)})", box=box.ROUNDED)
    ...
    # Show concatenated URLs
    console.print("\n[bold yellow]📋 CONCATENATED URLS:[/bold yellow]")
    console.print(Panel(explorer.get_concatenated_urls(), border_style="yellow"))
    
    # Option to save URLs to file
    if Confirm.ask("\nSave URLs to file?"):
        timestamp = int(time.time())
        filename = f"urls_{timestamp}.txt"
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(explorer.get_concatenated_urls())
        console.print(f"[green]✅ Saved URLs to {filename}[/green]")
...
def show_all_links(explorer: DOMExplorer):
    ...
    stats_table.add_row("Total",
                       ...
                       "")
    stats_table.add_row("ALL URLs",
                       str(len(link_map['all_urls'])),
                       "🌐")
    ...
    # Show ALL URLs (concatenated)
    console.print("\n[bold green]🌐 ALL URLS (CONCATENATED):[/bold green]")
    console.print(Panel(explorer.get_concatenated_urls(), border_style="green"))
...
def main():
    ...
    console.print("[dim]DOM Explorer | Clickable Elements | Link Mapper | Competition Detection | URL Extraction[/dim]")
    console.print()
    ...
    while True:
        ...
        console.print("  2. 🌐 Show ALL URLs (Concatenated)")
        ...
        if choice == "0":
        ...
        elif choice == "2":  # Show ALL URLs (Concatenated)
            show_all_urls(explorer)
            ...
            # Save structured data with competition analysis and all URLs
            link_map = explorer.map_links()
            competitor_analysis = explorer.get_competitor_analysis()
            ...
            json_data = {
                ...
                "text": text,
                "all_urls": explorer.get_all_urls(),
                "concatenated_urls": explorer.get_concatenated_urls()
            }
            ...
            # Also save URLs separately
            urls_filename = f"urls_{timestamp}.txt"
            with open(urls_filename, 'w', encoding='utf-8') as f:
                f.write(explorer.get_concatenated_urls())
            console.print(f"[green]✅ Saved URLs to {urls_filename}[/green]")
