...
Fixed WebSocket origin and connection issues
"""
...
class ChromeDevTools:
    """Chrome DevTools Protocol interface with fixed WebSocket connection"""
    ...
    def _get_websocket_connection(self, tab_id: str):
        """Get WebSocket connection with proper origin header"""
        ...
        try:
            ...
            console.print(f"[dim]WebSocket connection failed: {e}[/dim]")
            return None
...
class JavaScriptManager:
    """Manage JavaScript scripts with persistence"""
...
class ChromeSessionManager:
    ...
    def start_session(self, session_id: int):
        ...
        if session['status'] == 'running' and session['pid']:
            try:
                os.kill(session['pid'], 0)
                console.print(f"[yellow]Session '{session['name']}' is already running[/yellow]")
        ...
        # Chrome flags - simplified and with proper origin handling
        cmd = [
            self.chrome_path,
...
"""
    ...
    def manage_tabs_enhanced(self, session_id: int):
        """Enhanced interactive tab management"""
        session = self.db.get_session(session_id)
        if not session:
    ...
    def _manage_scripts(self, session_id: int):
        """Manage JavaScript scripts for a session"""
        console.clear()
...
╚══════════════════════════════════════════════════════════════╝
            ...
            status_line = f"📊 {total} sessions | 🟢 {running} running | 🔌 {available} ports | {display_status} | {root_status} | {script_count}"
