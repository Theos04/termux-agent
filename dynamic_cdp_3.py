...
class EnhancedChromeCDP:
        ...
        (function() {
            ...
            elements.forEach((el, index) => {
                ...
                results.push({
                    ...
                    xpath: null, // Will be computed if needed
                    // Store for click
                    canClick: true
                });
            });
    ...
    def interact_with_element(self, tab_index: int = 0, element_index: int = 0, 
        ...
        """Interact with a specific element using IIFE"""
        ...
        if action == 'click':
            ...
            (function() {{
                ...
                const isInput = el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.contentEditable === 'true';
                if (!isInput) {{
...
def main():
    ...
    while True:
        ...
        print(" 10. Find & Interact with Interactive Elements 🎯 NEW!")
        print(" 11. Generate Interaction Script - IIFE Approach 🚀 NEW!")
        print("  0. Exit")
        print("=" * 60)
        ...
        if choice == "0":
            ...
            while True:
                ...
                if cmd.lower() == 'q':
                    ...
                    if 0 <= elem_index < len(elements):
                        ...
                        while True:
                            print(f"\n🎮 Interact with element {elem_index}:")
                            print("  1. Click")
                            print("  2. Type text")
                            print("  3. Hover")
                            print("  4. Back")
                            ...
                            if action == '1':
                                chrome.interact_with_element(tab_index, elem_index, 'click')
                                break
                            elif action == '2':
                                text = input("Enter text to type: ")
                                chrome.interact_with_element(tab_index, elem_index, 'type', text)
                                break
                            elif action == '3':
                                chrome.interact_with_element(tab_index, elem_index, 'hover')
                                break
                            elif action == '4':
                                break
                    else:
            ...
            print("\n🚀 Generate Interaction Script (IIFE Approach)")
            print("-" * 60)
            ...
            elem_input = input("\n🔢 Enter element index to interact with (or 'all' for all): ").strip()
            
            if elem_input.lower() == 'all':
                # Generate script for all elements
                script_lines = [
                    "(function() {",
                    "    const actions = [];",
                    "    const results = [];",
                ...
                # Add actions for each element
                for i in range(len(elements)):
                    script_lines.append(f"    // Element {i}")
                    script_lines.append(f"    if (elements[{i}]) {{")
                    script_lines.append(f"        try {{")
                    script_lines.append(f"            const el = elements[{i}];")
                    script_lines.append(f"            el.click();")
                    script_lines.append(f"            results.push({{ success: true, index: {i} }});")
                    script_lines.append(f"        }} catch(e) {{")
                    script_lines.append(f"            results.push({{ success: false, index: {i}, error: e.message }});")
                ...
                if execute == 'y':
                    result = chrome.evaluate_script(full_script, tab_index)
                    if result:
                        print(f"✅ Execution results:")
                        for res in result:
                            status = "✅" if res.get('success') else "❌"
                            print(f"  {status} Element {res.get('index')}: {res.get('error', 'Success')}")
                
            elif elem_input.isdigit():
                elem_index = int(elem_input)
                if 0 <= elem_index < len(elements):
                    elem = elements[elem_index]
                    print(f"\n📋 Selected element: {elem.get('tag')}: {elem.get('text')[:50]}")
                    
                    # Generate specific interaction script
                    print("\n🎯 Choose action to script:")
                    print("  1. Click")
                    print("  2. Type text")
                    print("  3. Hover")
                    print("  4. Get element info")
                    action = input("Select action: ").strip()
                    ...
                    if action == '1':
                        ...
                        (function() {{
                            ...
                            const elements = document.querySelectorAll(selectors.join(','));
                            const el = elements[{elem_index}];
                            
                            if (!el) {{
                                return {{ error: 'Element not found' }};
                            }}
                            
                            // Simulate click with all necessary events
                            ...
                            // Also trigger native click if available
                        ...
                        execute = input("\n🔧 Execute this script? (y/n): ").strip().lower()
                        if execute == 'y':
                            result = chrome.evaluate_script(click_script, tab_index)
                            if result:
                        ...
                        execute = input("\n🔧 Execute this script? (y/n): ").strip().lower()
                        if execute == 'y':
                            result = chrome.evaluate_script(type_script, tab_index)
                            if result:
                                print(f"✅ Result: {json.dumps(result, indent=2)}")
                        ...
                        execute = input("\n🔧 Execute this script? (y/n): ").strip().lower()
                        if execute == 'y':
                            result = chrome.evaluate_script(hover_script, tab_index)
                            if result:
                                print(f"✅ Result: {json.dumps(result, indent=2)}")
                        ...
                        (function() {{
                            ...
                            return {{
                                ...
                                eventListeners: {{
                                    ...
                                    onchange: typeof el.onchange === 'function',
                                    oninput: typeof el.oninput === 'function',
                                    onmouseover: typeof el.onmouseover === 'function'
                                }}
                            }};
                        }})();
                        """
                        
                        print("\n📜 Generated IIFE Info Script:")
                        print("=" * 60)
                        print(info_script)
                        print("=" * 60)
                        ...
                        execute = input("\n🔧 Execute this script? (y/n): ").strip().lower()
                        if execute == 'y':
                            result = chrome.evaluate_script(info_script, tab_index)
                            if result:
                                print(f"\n✅ Element Info:")
