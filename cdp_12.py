#!/usr/bin/env python3
"""
Chrome Session Manager - Termux Final Working Edition with wscat
"""
...
class ChromeDevTools:
    ...
    def __init__(self, host='127.0.0.1', port=9222):
        ...
        if self.wscat_path:
            console.print("[dim]✅ Found wscat[/dim]")
    ...
    def execute_script(self, tab_id: str, script: str, return_by_value: bool = True) -> Optional[Dict]:
        ...
        try:
            ...
            # Try wscat first
            if self.wscat_path:
                result = self._execute_with_wscat(ws_url, script)
                if result:
                    return result
    ...
    def _execute_with_wscat(self, ws_url: str, script: str) -> Optional[Dict]:
        """Execute script using wscat"""
        try:
            ...
            with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json') as f:
                ...
                # Second message: Runtime.evaluate
                exec_msg = {
            ...
            try:
                ...
                # wscat -c ws://url -f messages.json
                cmd = [self.wscat_path, '-c', ws_url, '-f', temp_file]
                ...
                # Parse the output - wscat outputs each response on a new line
                if proc.stdout:
                    lines = proc.stdout.strip().split('\n')
                    # Look for the last response that contains 'result'
...
"""
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            header = """
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Termux wscat         ║
