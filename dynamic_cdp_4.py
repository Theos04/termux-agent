...
class EnhancedChromeCDP:
            ...
            elements.forEach((el, index) => {
                ...
                try {
                    hasClickHandler = typeof el.onclick === 'function' ||
                                     ...
                                     (el.getEventListeners && el.getEventListeners('click').length > 0);
                } catch(e) {
            ...
            if cmd.lower() == 'q':
                ...
                if action == '1':
                    for idx in selected_indices:
                        ...
                        self.interact_with_element(tab_index, idx, 'click')
    ...
    def get_element_info(self, tab_index: int = 0, element_index: int = 0) -> Optional[Dict]:
        ...
        (function() {{
            ...
            let listeners = [];
            try {{
                if (typeof el.getEventListeners === 'function') {{
                    listeners = el.getEventListeners('click');
                }}
            }} catch(e) {{}}
    ...
    def generate_interaction_script(self, tab_index: int = 0, indices: List[int] = None):
        """Generate an IIFE script for specified elements"""
        if not indices:
        ...
        for idx in indices:
            ...
            script_lines.append(f"        const el = elements[{idx}];")
            script_lines.append(f"        if (!el) {{")
            script_lines.append(f"            results.push({{ success: false, index: {idx}, error: 'Element not found' }});")
            script_lines.append(f"        }} else {{")
            script_lines.append(f"            // Scroll into view")
            script_lines.append(f"            el.scrollIntoView({{ behavior: 'smooth', block: 'center' }});")
            script_lines.append(f"            // Wait a moment for scroll to complete")
            script_lines.append(f"            await new Promise(r => setTimeout(r, 100));")
            script_lines.append(f"            // Trigger click with proper events")
            script_lines.append(f"            const clickEvent = new MouseEvent('click', {{")
            script_lines.append(f"                view: window,")
            script_lines.append(f"                bubbles: true,")
            script_lines.append(f"                cancelable: true")
            script_lines.append(f"            }});")
            script_lines.append(f"            el.dispatchEvent(clickEvent);")
            script_lines.append(f"            // Also try native click")
            script_lines.append(f"            if (typeof el.click === 'function') {{")
            script_lines.append(f"                el.click();")
            script_lines.append(f"            }}")
            script_lines.append(f"            results.push({{ success: true, index: {idx}, tag: el.tagName.toLowerCase() }});")
            script_lines.append(f"        }}")
            script_lines.append(f"    }} catch(e) {{")
            script_lines.append(f"        results.push({{ success: false, index: {idx}, error: e.message }});")
            script_lines.append(f"    }}")
            script_lines.append("")
        ...
        if execute == 'y':
            print("\n⚡ Executing script...")
            result = self.evaluate_script(full_script, tab_index)
            if result:
                print(f"\n📊 Execution Results:")
                for res in result:
                    if res.get('success'):
