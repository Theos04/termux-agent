#!/usr/bin/env python3
"""
Chrome Session Manager - Production v11
Fixed: Automatic display setup without manual VNC restart
"""
...
# ============================================================================
# Display Manager - Auto-creates X socket
# ============================================================================
...
class DisplayManager:
    ...
    def _ensure_x11_unix_dir(self) -> bool:
        """Create /tmp/.X11-unix directory if it doesn't exist"""
        x11_dir = "/tmp/.X11-unix"
        try:
            if not os.path.exists(x11_dir):
                os.makedirs(x11_dir, mode=0o1777, exist_ok=True)
                logger.info(f"Created {x11_dir}")
                return True
            return True
        except Exception as e:
            logger.error(f"Failed to create {x11_dir}: {e}")
            return False

    def _create_x_socket(self, display: str) -> bool:
        """Create X socket file for the display"""
        display_num = display.replace(':', '').replace('/', '')
        unix_socket = f"/tmp/.X11-unix/X{display_num}"

        try:
            # Create empty socket file if it doesn't exist
            if not os.path.exists(unix_socket):
                # Create a dummy socket file
                with open(unix_socket, 'w') as f:
                    f.write("")
                os.chmod(unix_socket, 0o777)
                logger.info(f"Created X socket: {unix_socket}")
                return True
            return True
        except Exception as e:
            logger.error(f"Failed to create X socket: {e}")
            return False

    def _setup_vnc_display(self) -> bool:
        """Setup VNC display properly"""
        display_num = self.config.vnc_display
        display = f":{display_num}"

        # Ensure X11 directory exists
        self._ensure_x11_unix_dir()

        # Create X socket
        self._create_x_socket(display)
        ...
        logger.info(f"✅ Setup display: {display}")
        console.print(f"[green]✅ Using display: {display} (port {self.vnc_port})[/green]")
        return True
    ...
    def _check_x_socket(self, display: str) -> bool:
        ...
        display_num = display.replace(':', '').replace('/', '')
        unix_socket = f"/tmp/.X11-unix/X{display_num}"
        return os.path.exists(unix_socket)

    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        """Get a working display - auto-creates X socket"""

        # First, ensure X11 directory exists
        self._ensure_x11_unix_dir()
        ...
        if env_display:
            ...
            if not self._check_x_socket(env_display):
                self._create_x_socket(env_display)
...
class ChromeSessionManager:
    def __init__(self):
        ...
        # Initialize display - NOW AUTOMATIC
...
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Production v11       ║
║        Auto Display Setup | No Manual VNC Restart         ║
╚══════════════════════════════════════════════════════════════╝
            """
