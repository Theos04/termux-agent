#!/usr/bin/env python3
"""
Chrome Session Manager - Production v10
Fixed: Direct display handling without VNC restart
"""
...
# ============================================================================
# Display Manager - Fixed: Simple direct display check
# ============================================================================
...
class DisplayManager:
    ...
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        """Get a working display - simple and direct"""
        
        # Check if DISPLAY env var is set
        env_display = os.environ.get('DISPLAY')
        if env_display:
            logger.info(f"DISPLAY env var: {env_display}")
            # Even if socket doesn't exist, try to use it
            # Chrome might work with just the display number
        ...
        for display_num in range(self.config.display_start, self.config.display_end + 1):
            display = f":{display_num}"
            # Try to use the display even if socket doesn't exist
            self.current_display = display
            os.environ['DISPLAY'] = display
            logger.info(f"Trying display: {display}")
            return display, None
...
class ChromeLauncher:
    ...
    def build_command(self, session: Dict, use_display: bool) -> List[str]:
        ...
        if use_display:
            # Use the URL directly - Chrome will open in the display
                ...
                if log_file.exists():
                    with open(log_file, 'r') as f:
                        ...
                        if "cannot open display" in error_msg.lower():
                            console.print("[red]❌ Chrome cannot open display. Please check VNC.[/red]")
                            console.print("[yellow]💡 VNC is running but X server not responding[/yellow]")
                            console.print("[yellow]💡 Try: export DISPLAY=:1 && vncserver :1[/yellow]")
