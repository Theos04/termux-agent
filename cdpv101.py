...
class WSCatWebSocket:
    """WebSocket handler using wscat for reliable connections"""
    ...
    def execute_script(self, script: str, return_by_value: bool = True) -> Optional[Dict]:
        ...
        result = self.send_command(enable_cmd)
...
class ChromeDevTools:
    ...
    def execute_script(self, tab_id: str, script: str, return_by_value: bool = True) -> Optional[Dict]:
        result = self.execute_script_wscat(tab_id, script)
        if result:
            return result
    ...
    def get_page_html(self, tab_id: str) -> Optional[str]:
        script = """
        (function() {
    ...
    def get_ws_info(self, tab_id: str) -> Optional[Dict]:
        tab = self.get_tab_by_id(tab_id)
        if tab:
            return {
                'id': tab.get('id'),
...
class WSCatManager:
    ...
    def connect_and_listen(self, ws_url: str, timeout: int = 5) -> List[Dict]:
        messages = []
        try:
            cmd = [
                'wscat',
            ...
            process.stdin.write(json.dumps(enable_cmd) + '\n')
            process.stdin.flush()
            ...
            console.print(f"[red]wscat error: {e}[/red]")
...
class ChromeSessionManager:
    ...
    def start_session(self, session_id: int):
        ...
        # FIXED: Chrome flags for Termux with error suppression
        cmd = [
            self.chrome_path,
            ...
            # Critical: Suppress inotify and DBus errors
            ...
            "--disable-dbus",  # Disable DBus entirely
        ...
        try:
            ...
            # Set environment to suppress error messages
            env['CHROME_LOG_FILE'] = '/dev/null'  # Redirect chrome logs to null
            env['G_MESSAGES_DEBUG'] = ''  # Suppress GLib messages
            env['DBUS_SESSION_BUS_ADDRESS'] = '/dev/null'  # Disable DBus
            env['GTK_MODULES'] = ''  # Disable GTK modules
            ...
            if process.poll() is not None:
                ...
                # Try to get more info by running without redirection
                try:
...
"""
    ...
    def show_ws_info(self, session_id: int):
        ...
        for i, tab in enumerate(tabs, 1):
            ws_info = devtools.get_ws_info(tab['id'])
            if ws_info:
                ws_id = self.wscat_manager.get_ws_id(ws_info['ws_url'])
                table.add_row(
                    f"Tab {i}",
                    ws_id[:8],
                    ws_info['ws_url'][:50] + "...",
        ...
        # Also show wscat test
        console.print("\n[cyan]Testing wscat connection...[/cyan]")
        if tabs:
            test_tab = tabs[0]
            ws_url = devtools.get_tab_ws_url(test_tab['id'])
            if ws_url:
                console.print(f"[dim]Connecting to: {ws_url}[/dim]")
                messages = self.wscat_manager.connect_and_listen(ws_url, timeout=2)
                if messages:
                    console.print(f"[green]✅ Received {len(messages)} messages via wscat[/green]")
                else:
                    console.print("[yellow]⚠️ No messages received (might need active page)[/yellow]")
