#!/usr/bin/env python3
"""
Chrome Session Manager - Production v9
Fixed: Direct VNC display usage without Xvfb
"""
...
# ============================================================================
# Display Manager - Fixed: Uses VNC display directly
# ============================================================================
...
class DisplayManager:
    ...
    def _check_x_socket(self, display: str) -> bool:
        ...
        logger.debug(f"✗ No X socket found for {display}")
        return False
    
    def _start_vnc_xserver(self, display: str) -> bool:
        """Try to start the VNC X server using vncserver command"""
        display_num = display.replace(':', '')
        try:
            logger.info(f"Attempting to start VNC X server on {display}")
            ...
            # Check if X socket exists now
            if self._check_x_socket(display):
                logger.info(f"✅ VNC X server started on {display}")
                return True
            else:
                logger.warning(f"VNC server started but X socket not found")
                return False
                
        except Exception as e:
            logger.error(f"Failed to start VNC X server: {e}")
            return False
    
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        """Get a working display - uses VNC directly"""
        
        # 1. Check if DISPLAY env var is set and working
        ...
        if env_display:
            ...
            if self._check_x_socket(env_display):
                ...
                # Try to start VNC X server
                if self._start_vnc_xserver(env_display):
                    self.current_display = env_display
                    console.print(f"[green]✅ Started VNC X server on {env_display}[/green]")
                    return env_display, None
        ...
        for session in vnc_sessions:
            ...
            if self._check_x_socket(display):
                ...
                logger.warning(f"VNC session {display} exists but X socket missing")
                # Try to restart VNC with X
                if self._start_vnc_xserver(display):
                    self.current_display = display
                    self.vnc_port = session['port']
                    self.vnc_pid = session['pid']
                    os.environ['DISPLAY'] = display
                    console.print(f"[green]✅ Restarted VNC X server on {display} (port {session['port']})[/green]")
                    console.print(f"[green]📺 VNC available: vnc://127.0.0.1:{session['port']} (password: {self.config.vnc_password})[/green]")
                    return display, session['port']
        
        # 3. Try to start a new VNC session
        for display_num in range(self.config.display_start, self.config.display_end + 1):
            ...
            if not self._check_x_socket(display):
                if self._start_vnc_xserver(display):
                    ...
                    os.environ['DISPLAY'] = display
                    logger.info(f"Started VNC X server on {display}")
                    console.print(f"[green]✅ Started VNC X server on {display} (port {self.vnc_port})[/green]")
...
class ChromeLauncher:
    ...
    def _launch_once(self, session: Dict) -> Tuple[bool, Optional[int], str]:
        ...
        if display:
            # Check if X socket exists - if not, try to start VNC X server
            if self.display_manager._check_x_socket(display):
                ...
                logger.warning(f"Display {display} socket missing, attempting to start VNC X server")
                if self.display_manager._start_vnc_xserver(display):
                    use_display = True
                ...
                if log_file.exists():
                    with open(log_file, 'r') as f:
                        ...
                        if "cannot open display" in error_msg.lower():
                            console.print("[red]❌ Chrome cannot open display. Restart VNC manually:[/red]")
                            console.print("[yellow]💡 vncserver -kill :1 && vncserver :1[/yellow]")
...
# ============================================================================
# Main Session Manager (full implementation)
# ============================================================================
...
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Production v9        ║
║        VNC Direct | Auto-Restart X Server                ║
╚══════════════════════════════════════════════════════════════╝
            """
