# scraper.py - Complete robust version with fixed extraction
import asyncio
import json
import time
import logging
...
class UnstopJobScraper:
    ...
    def _extract_value(self, result: Any, default: Any = None) -> Any:
        """Helper to extract value from Chrome client response"""
        if result is None:
            return default
        if isinstance(result, dict):
            ...
            if isinstance(value, dict):
                ...
                # If inner is a primitive, return it
                if inner is not None:
                    return inner
            # If value is a primitive, return it
            if value is not None and not isinstance(value, dict):
                return value
            # Check for direct 'value' key at top level
            if 'value' in result:
    ...
    def wait_for_page_load(self, timeout: int = 30) -> bool:
        ...
        while time.time() - start < timeout:
            try:
                ...
                state = self._extract_value(result)
                if state == 'complete':
                    return True
            except:
                pass
    ...
    def wait_for_content_update(self, previous_count: int, timeout: int = 60) -> bool:
        ...
        while time.time() - start < timeout:
            try:
                ...
                count = self._extract_value(card_count, 0)
                ...
                loading_element = self._extract_value(loading)
                is_loading = loading_element is not None
    ...
    def scroll_to_load_all(self, max_scrolls: int = 10):
        ...
        for i in range(max_scrolls):
            try:
                ...
                scroll_data = self._extract_value(result, {})
                at_bottom = scroll_data.get('atBottom', False) if isinstance(scroll_data, dict) else False
    ...
    def get_pagination_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result_value = self._extract_value(result, '{}')
            
            # If we got a primitive (not a string or dict), return default
            if not isinstance(result_value, (str, dict)):
                logger.warning(f"Pagination result is not string or dict: {type(result_value)}")
            ...
            if isinstance(result_value, str):
                try:
                    ...
                    if isinstance(parsed, dict):
                        ...
                        logger.warning(f"Pagination parsed result is not dict: {type(parsed)}")
                        return {'has_pagination': False, 'current_page': 1, 'total_pages': 1, 'has_next': False}
                except json.JSONDecodeError as e:
                    # Check if the string is 'complete' (from readyState) and ignore
                    if result_value == 'complete':
                        return {'has_pagination': False, 'current_page': 1, 'total_pages': 1, 'has_next': False}
    ...
    def click_pagination_button(self, target_page: int) -> bool:
        ...
        try:
            result = self.client.evaluate(js_code)
            clicked = self._extract_value(result, False)
            if clicked:
    ...
    def extract_page_jobs(self, page_number: int = 1) -> Dict[str, Any]:
        ...
        try:
            result = self.client.evaluate(js_code)
            extracted_raw = self._extract_value(result, '{}')
            
            # If we got an int or other primitive, return empty
            if not isinstance(extracted_raw, (str, dict)):
                logger.error(f"Extracted result is not dict or string: {type(extracted_raw)}")
                return {'jobs': []}
            
            # Try to parse as JSON if it's a string
            if isinstance(extracted_raw, str):
                # Check if it's a primitive string like 'complete'
                if extracted_raw in ['complete', 'loading', 'interactive']:
                    logger.warning(f"Got page state string instead of job data: {extracted_raw}")
                    return {'jobs': []}
                try:
                    extracted = json.loads(extracted_raw)
                except json.JSONDecodeError:
                    logger.error(f"Failed to parse extraction JSON: {extracted_raw[:100]}")
                    return {'jobs': []}
            ...
            if not isinstance(extracted, dict):
                logger.error(f"Extracted result is not a dict: {type(extracted)}")
                return {'jobs': []}
    ...
    def scrape_page(self, page_number: int, initial_load: bool = False) -> Optional[JobPage]:
        ...
        if initial_load:
            ...
            try:
                ...
                count_before = self._extract_value(current_count, 0)
                try:
