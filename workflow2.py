#!/usr/bin/env python3
"""
workflow1.py - Complete Workflow Template Engine (Fixed with Python Script Support)
"""
...
class WorkflowConfig:
    ...
    python_scripts_dir: str = os.path.expanduser("~/python-scripts")
...
class PythonScriptManager:
    """Manages Python scripts for execution in workflows"""
    ...
    def _load_scripts(self):
        """Load all Python scripts from directory"""
        self.scripts = {}
        self.categories = {}
        ...
        console.print(f"[green]📚 Loaded {len(self.scripts)} Python scripts from {len(self.categories)} categories[/green]")
    
    def _load_script_file(self, file_path: Path) -> Optional[Dict]:
        """Load a Python script file with metadata"""
        try:
            ...
            # Check if script has a main function or is executable
    ...
    def _extract_metadata(self, content: str) -> Dict:
        ...
        if lines and lines[0].startswith('"""'):
            ...
            if docstring:
                metadata['description'] = ' '.join(docstring)
        ...
        for line in lines[:30]:
            if line.startswith('#') and ':' in line:
                ...
                if len(parts) == 2:
                    key = parts[0].strip()
                    value = parts[1].strip()
                    if key in ['name', 'description', 'author', 'version', 'requires']:
                        metadata[key] = value
    ...
    def execute_script(self, script_id: str, args: List[str] = None, 
                       env_vars: Dict = None, timeout: int = 300) -> Dict:
        ...
        if not os.path.exists(file_path):
            raise ValueError(f"Script file '{file_path}' not found")
    ...
    def execute_script_by_path(self, file_path: str, args: List[str] = None,
                               env_vars: Dict = None, timeout: int = 300) -> Dict:
        """Execute a Python script by file path"""
        path = Path(file_path)
        if not path.exists():
            raise ValueError(f"Script file '{file_path}' not found")
        
        if not path.suffix == '.py':
            raise ValueError(f"File '{file_path}' is not a Python script")
    ...
    def display_scripts(self):
        ...
        if not self.scripts:
            console.print("[dim]No Python scripts found in library[/dim]")
            return
            
        tree = Tree("[bold cyan]🐍 Python Script Library[/bold cyan]")
        ...
        for category in sorted(self.categories.keys()):
            ...
            for script_id in script_ids:
                ...
                if script:
                    ...
                    has_main = " [dim](has main)[/dim]" if script.get('has_main') else ""
                    category_tree.add(f"[white]{script['name']}{has_main}[/white][dim]{desc}[/dim]")
        
        console.print(tree)

# ============================================================================
# Script Library Manager (JS)
# ============================================================================
...
class ScriptLibrary:
    ...
    def _load_scripts(self):
        ...
        console.print(f"[green]📚 Loaded {len(self.scripts)} JavaScript scripts from {len(self.categories)} categories[/green]")
    ...
    def display_scripts(self):
        """Display scripts in a nice tree format"""
        if not self.scripts:
            console.print("[dim]No JavaScript scripts found in library[/dim]")
            return
            
        tree = Tree("[bold cyan]📜 JavaScript Script Library[/bold cyan]")
...
class StepType(Enum):
    ...
    EXECUTE_PYTHON_PATH = "execute_python_path"
    WAIT = "wait"
...
class WorkflowEngine:
    ...
    def __init__(self, config: WorkflowConfig):
        ...
        self.python_script_manager = PythonScriptManager(config.python_scripts_dir)
    ...
    def _execute_step(self, step: WorkflowStep, context: Dict) -> Dict:
        ...
        try:
            ...
            handler = {
                ...
                StepType.EXECUTE_PYTHON: self._handle_execute_python,
                StepType.EXECUTE_PYTHON_PATH: self._handle_execute_python_path,
    ...
    def _handle_execute_python(self, step: WorkflowStep, context: Dict) -> Dict:
        """Execute a Python script from library"""
        params = step.params
        script_id = params.get('script_id')
        ...
        args = self._replace_template_vars(args, context)
        
        # Execute script
        result = self.python_script_manager.execute_script(
            script_id, args, env_vars, timeout
        )
        
        # Store output in context
        if result.get('success'):
            context['context']['python_output'] = result.get('stdout', '')
        
        return result
    
    def _handle_execute_python_path(self, step: WorkflowStep, context: Dict) -> Dict:
        """Execute a Python script by file path"""
        params = step.params
        ...
        # Replace template variables in file_path and args
        file_path = self._replace_template_vars(file_path, context)
        args = self._replace_template_vars(args, context)
        
        # Execute script
        result = self.python_script_manager.execute_script_by_path(
            file_path, args, env_vars, timeout
        )
    ...
    def _replace_template_vars(self, value: Any, context: Dict) -> Any:
        """Replace template variables in a value"""
        if isinstance(value, str):
            # Replace {execution_id}
            value = value.replace('{execution_id}', context.get('execution_id', ''))
            value = value.replace('{workflow_name}', context.get('workflow_name', ''))
            ...
            # Replace context variables
            for key, val in context.get('context', {}).items():
                value = value.replace(f'{{{{{key}}}}}', str(val))
            
            return value
        elif isinstance(value, list):
            return [self._replace_template_vars(item, context) for item in value]
        elif isinstance(value, dict):
            return {k: self._replace_template_vars(v, context) for k, v in value.items()}
        else:
            return value
...
class WorkflowBuilder:
    ...
    def _build_steps(self):
        ...
        while True:
            ...
            step_types = [
                ...
                ("6", "📜 Execute JS Script", "Run JS script from library"),
                ("7", "🐍 Execute Python", "Run Python script from library"),
                ("8", "📁 Execute Python Path", "Run Python script by file path"),
                ("9", "⏳ Wait", "Wait for seconds"),
                ("10", "📊 Extract Data", "Extract data from page"),
                ("11", "📸 Screenshot", "Take screenshot"),
                ("12", "🔗 Webhook", "Send webhook"),
                ("13", "🔔 Notify", "Send notification"),
            ]
            ...
            choice = Prompt.ask("Select step type", 
    ...
    def _add_step_interactive(self, choice: int):
        ...
        step_type_map = {
            ...
            7: StepType.EXECUTE_PYTHON,
            8: StepType.EXECUTE_PYTHON_PATH,
            9: StepType.WAIT,
            10: StepType.EXTRACT_DATA,
            11: StepType.SCREENSHOT,
            12: StepType.WEBHOOK,
            13: StepType.NOTIFY,
        }
        ...
        if step_type == StepType.START_SESSION:
            ...
            console.print("\n[bold]Available JavaScript Scripts:[/bold]")
            ...
            console.print("\n[bold]Available Python Scripts:[/bold]")
            self.engine.python_script_manager.display_scripts()
            
            script_id = Prompt.ask("Python script ID or name to execute")
            # Try to find by ID or name
            script = self.engine.python_script_manager.get_script(script_id)
            if not script:
                # Search by name
                for sid, s in self.engine.python_script_manager.scripts.items():
                    if script_id.lower() in s['name'].lower():
                        script = s
                        script_id = sid
                        break
            ...
            # Optional arguments
            if Confirm.ask("Add command line arguments?"):
                args_str = Prompt.ask("Arguments (space separated)")
                params['args'] = args_str.split()
        
        elif step_type == StepType.EXECUTE_PYTHON_PATH:
            file_path = Prompt.ask("Full path to Python script")
            if os.path.exists(file_path):
                params['file_path'] = file_path
                console.print(f"[green]✓ Script found at: {file_path}[/green]")
            else:
...
class WorkflowCLI:
    ...
    def run(self):
        ...
        while True:
            ...
            menu.add_row("10", "🐍 Python Scripts", "Manage Python scripts")
    ...
    def _select_step_type(self) -> Optional[StepType]:
        ...
        types = [
            ...
            ("6", StepType.EXECUTE_SCRIPT, "Execute JS Script"),
            ("7", StepType.EXECUTE_PYTHON, "Execute Python"),
            ("8", StepType.EXECUTE_PYTHON_PATH, "Execute Python Path"),
            ("9", StepType.WAIT, "Wait"),
            ("10", StepType.EXTRACT_DATA, "Extract Data"),
            ("11", StepType.SCREENSHOT, "Screenshot"),
            ("12", StepType.WEBHOOK, "Webhook"),
            ("13", StepType.NOTIFY, "Notify"),
        ]
    ...
    def _collect_step_params(self, step_type: StepType) -> Dict:
        ...
        if step_type == StepType.START_SESSION:
            ...
            params['script_id'] = Prompt.ask("JavaScript Script ID")
        
        elif step_type == StepType.EXECUTE_PYTHON:
            params['script_id'] = Prompt.ask("Python Script ID")
            if Confirm.ask("Add arguments?"):
                args_str = Prompt.ask("Arguments (space separated)")
                params['args'] = args_str.split()
        
        elif step_type == StepType.EXECUTE_PYTHON_PATH:
            params['file_path'] = Prompt.ask("Full path to Python script")
            if Confirm.ask("Add arguments?"):
                args_str = Prompt.ask("Arguments (space separated)")
                params['args'] = args_str.split()
    ...
    def _browse_scripts(self):
        ...
        console.print("\n[bold]📚 JavaScript Script Library[/bold]")
        self.engine.script_library.display_scripts()
    ...
    def _manage_python_scripts(self):
        """Manage Python scripts"""
        console.clear()
        console.print(Panel("[bold cyan]🐍 Python Script Manager[/bold cyan]", border_style="green"))
        
        # Show Python scripts
        self.engine.python_script_manager.display_scripts()
        
        console.print("\n[bold]Options:[/bold]")
        console.print("  1. Execute Python script")
        console.print("  2. Execute by file path")
        console.print("  3. Search Python scripts")
        console.print("  4. Refresh script list")
        ...
        if choice == "1":
            ...
            if script_id:
                ...
                result = self.engine.python_script_manager.execute_script(script_id, args)
                ...
                if result['success']:
                    ...
                    console.print(f"[red]❌ Script failed: {result.get('error', 'Unknown error')}[/red]")
        
        elif choice == "2":
            file_path = Prompt.ask("Enter full path to Python script")
            if file_path and os.path.exists(file_path):
                args_str = Prompt.ask("Arguments (space separated)", default="")
                args = args_str.split() if args_str else []
                
                result = self.engine.python_script_manager.execute_script_by_path(file_path, args)
            ...
            query = Prompt.ask("Search term")
            results = self.engine.python_script_manager.search_scripts(query)
            ...
            if results:
                ...
                table.add_column("Description", style="dim")
                table.add_column("Category", style="cyan")
                table.add_column("Has Main", style="yellow", width=8)
            ...
            self.engine.python_script_manager._load_scripts()
            console.print("[green]✅ Script list refreshed[/green]")
