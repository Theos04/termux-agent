# scraper.py - Complete robust version with fixed value extraction
import asyncio
import json
import time
import logging
    ...
    def _extract_value(self, result: Any, default: Any = None) -> Any:
        ...
        if isinstance(result, dict):
            ...
            if isinstance(value, dict):
                ...
                # If value is a primitive (string, int, etc.), return it
                if value is not None:
                    return value
            # If value is a primitive, return it
            if value is not None:
                return value
            # Fallback to trying to get 'value' directly
            if 'value' in result:
    ...
    def extract_page_jobs(self, page_number: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(extracted_raw, str):
                ...
                logger.error(f"Extracted result is not a dict or string: {type(extracted_raw)}")
                return {'jobs': []}
