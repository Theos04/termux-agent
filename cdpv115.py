#!/usr/bin/env python3
"""
Chrome Session Manager - Production v12
Fixed: Typo in menu call and better /tmp handling
"""
...
# ============================================================================
# Display Manager - Fixed for Termux /tmp handling
# ============================================================================
...
class DisplayManager:
    ...
    def _check_x_socket(self, display: str) -> bool:
        """Check if X socket exists - handles Termux's read-only /tmp"""
        display_num = display.replace(':', '').replace('/', '')
        
        # Try different possible locations for X socket
        possible_paths = [
            f"/tmp/.X11-unix/X{display_num}",
            f"/data/data/com.termux/files/usr/tmp/.X11-unix/X{display_num}",
            f"{os.environ.get('TMPDIR', '/tmp')}/.X11-unix/X{display_num}"
        ]
        
        for path in possible_paths:
            if os.path.exists(path):
                logger.debug(f"Found X socket at: {path}")
                return True
        
        return False

    def _setup_vnc_display(self) -> bool:
        """Setup VNC display - uses DISPLAY env var directly"""
        display_num = self.config.vnc_display
        display = f":{display_num}"

        # Set DISPLAY environment - this is the most reliable way in Termux
    ...
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        """Get a working display - uses DISPLAY env var"""
        ...
        if env_display:
            ...
            for session in vnc_sessions:
                if session['display'] == env_display:
            ...
            return env_display, self.vnc_port
...
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Production v12       ║
║        Auto Display | VNC Ready | Fixed Menu             ║
╚══════════════════════════════════════════════════════════════╝
            """
