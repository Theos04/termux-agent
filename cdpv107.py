#!/usr/bin/env python3
"""
Chrome Session Manager - Production v5
Fixed: Proper display detection and VNC integration
"""
...
class DisplayManager:
    ...
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        ...
        if env_display and self._check_display_works(env_display):
        ...
        for session in vnc_sessions:
            ...
            logger.info(f"Found VNC session: {display} (port {session['port']})")
...
class ChromeLauncher:
    ...
    def _launch_once(self, session: Dict) -> Tuple[bool, Optional[int], str]:
        ...
        if self.display_manager.current_display:
            # Verify display is still working
            try:
                env = os.environ.copy()
                ...
                if result.returncode == 0:
                    ...
                    console.print(f"[green]🖥️ Using display: {self.display_manager.current_display}[/green]")
                else:
                    logger.warning(f"Display {self.display_manager.current_display} not responding")
            except Exception as e:
...
class ChromeSessionManager:
    def __init__(self):
        ...
        if self.display:
            ...
            if self.vnc_port:
                console.print(f"[green]✅ VNC on port {self.vnc_port} (password: {self.config.vnc_password})[/green]")
        else:
