#!/usr/bin/env python3
"""
Chrome Session Manager - HTTP-based approach (No WebSocket required)
"""
...
class ChromeDevTools:
    """Chrome DevTools Protocol using HTTP requests only"""
    ...
    def execute_script(self, tab_id: str, script: str, return_by_value: bool = True) -> Optional[Dict]:
        """
        Execute JavaScript using HTTP requests via the Chrome DevTools Protocol.
        This uses a simple approach - we navigate to a page with the script in the URL.
        """
        try:
            ...
            # Method 1: Use CDP via HTTP - simpler but limited
            # We'll use the /json/new endpoint with a javascript: URL
            # This is a workaround for WebSocket issues
            
            # Create a new tab with the JavaScript as a URL
            # This will execute the script immediately
            js_url = f"javascript:(function(){{ {script} }})()"
            
            # Open a new tab with the JavaScript URL
            ...
            if response.status_code == 200:
                # The script executed, now close the tab
                new_tab = response.json()
                tab_id_to_close = new_tab.get('id')
                if tab_id_to_close:
                    # Wait a moment for the script to execute
                    time.sleep(1)
                    self.close_tab(tab_id_to_close)
                    return {"value": "Script executed successfully"}
            
            # Method 2: Use the page that's already open
            # We'll try to use the navigate_to method which uses JavaScript in the URL
            # But with a different approach - using the existing tab
            
            # Get the current URL
            current_url = tab.get('url', '')
            
            # Navigate to a javascript: URL and back
            # This is a workaround for executing JS without WebSocket
            try:
                # First, navigate to the javascript: URL
                js_url_encoded = f"javascript:(function(){{ {script} }})()"
                response = self.session.post(
                    f"{self.base_url}/json/activate/{tab_id}",
                    timeout=5
                )
                
                # Try to navigate using the /json/new approach with the existing tab
                # We'll create a new tab with the JavaScript and then close it
                # The JavaScript will run in the context of the new tab
                
                # This is the most reliable method - create a new tab with the script
                response = self.session.post(
                    f"{self.base_url}/json/new",
                    params={'url': js_url_encoded},
                    timeout=10
                )
            ...
            console.print("[red]Failed to execute script[/red]")
            return None
    ...
    def get_page_content(self, tab_id: str) -> Optional[str]:
        """Get page content using a simple approach"""
        try:
            tab = self.get_tab_by_id(tab_id)
            if not tab:
                return None
            
            # Use the /json endpoint to get the page content
            # The tab object already has the URL, but we need the actual content
            # We'll use a different approach - fetch the page via HTTP
    ...
    def get_page_text(self, tab_id: str) -> Optional[str]:
        """Get text from page using a simple approach"""
    ...
    def get_page_metadata(self, tab_id: str) -> Dict:
        ...
        if tab:
            return {
                ...
                'contentLength': len(tab.get('url', ''))
            }
        return {}

    def navigate_to(self, tab_id: str, url: str) -> bool:
        """Navigate to a URL - this works via HTTP"""
        try:
            # Use the /json/activate endpoint to navigate
            response = self.session.post(
                f"{self.base_url}/json/activate/{tab_id}",
                timeout=5
            )
            if response.status_code == 200:
                # Now navigate using the /json/new approach
                # Actually, we need to use the /json/new with the URL
                # But we want to use the existing tab
                # We'll create a new tab with the URL and close the old one
                response = self.session.post(
                    f"{self.base_url}/json/new",
                    params={'url': url},
                    timeout=5
    ...
    def get_cookies(self, tab_id: str) -> Optional[List[Dict]]:
        """Get cookies - this doesn't work without WebSocket"""
        return []

    def get_local_storage(self, tab_id: str) -> Optional[Dict]:
        """Get localStorage - this doesn't work without WebSocket"""
        return {}

    def get_session_storage(self, tab_id: str) -> Optional[Dict]:
        """Get sessionStorage - this doesn't work without WebSocket"""
        return {}

    def click_element(self, tab_id: str, selector: str) -> bool:
        """Click element using JavaScript injection via URL"""
        script = f"""
        (function() {{
    ...
    def fill_input(self, tab_id: str, selector: str, value: str) -> bool:
        """Fill input using JavaScript injection via URL"""
        script = f"""
        (function() {{
            const el = document.querySelector('{selector}');
            if (el) {{
...
"""
    ...
    def interactive_menu(self):
        ...
        while True:
            ...
            header = """
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - HTTP Edition        ║
║           No WebSocket Required - Works on Termux         ║
╚══════════════════════════════════════════════════════════════╝
            """
