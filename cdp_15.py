#!/usr/bin/env python3
"""
Chrome Session Manager - Working HTTP Edition
Uses HTTP requests only - NO WebSocket required
"""
...
class ChromeDevTools:
    ...
    def execute_script(self, tab_id: str, script: str, return_by_value: bool = True) -> Optional[Dict]:
        """Execute JavaScript using the javascript: URL approach"""
        try:
            # Method: Create a new tab with the JavaScript as a URL
            # This is the most reliable way without WebSocket
            js_encoded = urllib.parse.quote(script)
            js_url = f"javascript:(function(){{ {script} }})();"
            
            # Create a new tab with the JavaScript URL
            ...
            if response.status_code == 200:
                # The script executed in the new tab
            ...
            # Alternative: Try using the existing tab with a different approach
            # Some Chrome versions support executing script via the /json endpoint
            try:
                # Get the tab info
                tab = self.get_tab_by_id(tab_id)
                if tab:
                    current_url = tab.get('url', '')
                    # Try to navigate to a javascript: URL and back
                    # First, navigate to javascript: URL
    ...
    def click_element(self, tab_id: str, selector: str) -> bool:
        """Click element using JavaScript execution via URL"""
        script = f"""
        (function() {{
    ...
    def fill_input(self, tab_id: str, selector: str, value: str) -> bool:
        """Fill input using JavaScript execution via URL"""
        script = f"""
        (function() {{
            const el = document.querySelector('{selector}');
            if (el) {{
