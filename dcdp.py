...
class EnhancedChromeCDP:
    ...
    def get_computed_styles(self, tab_index: int = 0, 
        ...
        try:
            ...
            # Get document on the SAME connection
            params = {"depth": 1, "pierce": True}  # Only get first level for speed
            result = self._send_cdp_command(ws, "DOM.getDocument", params)
            ...
            # Find first element node from this document
            element_id = self._find_first_element_in_children(root)
            if not element_id:
                print("❌ Could not find any element nodes")
                ws.close()
                return None
            
            print(f"   Using element node ID: {element_id}")
            
            params = {"nodeId": element_id}
    ...
    def _find_first_element_in_children(self, node: Dict) -> Optional[int]:
        """Find the first element node (nodeType 1) in the DOM tree"""
        # Check children first (skip document node)
        for child in node.get('children', []):
            if child.get('nodeType') == 1:  # Element node
                return child.get('nodeId')
        
        # If no direct children are elements, search deeper
        for child in node.get('children', []):
            result = self._find_first_element_in_children(child)
            if result:
                return result
