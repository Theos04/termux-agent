#!/usr/bin/env python3
"""
Enhanced Real-time Stock Monitor for MoneyControl
Fixed bugs + Advanced features: OI analysis, volume spikes, momentum scoring
"""
...
class EnhancedStockMonitor:
    """Enhanced real-time stock monitor with advanced analytics"""
    ...
    def __init__(self, check_interval: float = 2.0):
        ...
        # Historical data storage - FIXED: separate price and volume histories
        self.price_history = defaultdict(lambda: deque(maxlen=100))
        self.volume_history = defaultdict(lambda: deque(maxlen=100))
        self.volume_delta_history = defaultdict(lambda: deque(maxlen=60))  # NEW: track changes
        ...
        self.min_volume_threshold = 1_000_000  # Minimum volume for liquidity filter
    ...
    def fetch_all_stocks(self) -> List[Dict]:
        """Fetch stocks with F&O categorization - FIXED: merge categories"""
        categories = ["Active Buying", "Strong Buying", "Short Covering",
                     "Active Selling", "Strong Selling", "Profit Booking"]

        stock_map = {}  # Use dict to merge duplicates
    ...
    def calculate_volume_spike(self, stock_name: str, current_volume: float) -> Optional[float]:
        """FIXED: Calculate volume spike using deltas instead of cumulative volume"""
        # Store current volume for delta calculation
        ...
        avg_delta = sum(deltas[:-1]) / (len(deltas) - 1)  # Exclude current
        if avg_delta > 0 and delta > avg_delta * self.volume_spike_threshold:
            return delta / avg_delta  # Return spike multiplier
        
        return None

    def check_price_movement(self, stock_name: str, current_price: float) -> Dict:
        """FIXED: Compare with previous price before appending"""
        history = self.price_history[stock_name]
        
        if len(history) < 1:
            return {}
            
        old_price = history[-1]  # Previous price (before appending current)
    ...
    def analyze_oi_pattern(self, stock_name: str, price: float, oi: float) -> Dict:
        """NEW: OI-based pattern analysis"""
        if stock_name not in self.oi_history or len(self.oi_history[stock_name]) < 2:
            self.oi_history[stock_name].append(oi)
            return {}
            
        prev_oi = self.oi_history[stock_name][-1]
        prev_price = self.price_history[stock_name][-1] if stock_name in self.price_history else price
    ...
    def calculate_momentum_score(self, stock: Dict) -> float:
        """NEW: Composite momentum score"""
        price_change = stock.get("pricePerChange", 0)
        volume = stock.get("volume", 0)
        oi = stock.get("openInt", 0)
        price = stock.get("price", 0)
        
        # Normalize components (rough scoring)
        price_score = max(-1, min(1, price_change / 10))  # -1 to 1 for ±10%
        
        # Volume score based on relative liquidity
        volume_score = min(1, volume / 50_000_000)  # Cap at 50M
        
        # OI score based on build-up
        oi_score = 0
        if stock_name in self.oi_history:
            oi_hist = list(self.oi_history[stock_name])
            if len(oi_hist) > 1:
        ...
        score = (
            ...
            0.25 * volume_score +
            0.20 * oi_score +
            0.10 * min(1, volume * price / 1_000_000_000) +  # Turnover (billions)
            0.10 * cat_score
        )
    ...
    def display_status(self, current_time: str, stocks: List[Dict],
        ...
        for i, stock in enumerate(sorted_by_change[:5], 1):
            ...
            print(f"  {i}. {name:20} ₹{price:>8.2f}  {Fore.GREEN}{change:>+6.2f}%{Style.RESET_ALL}  {pattern_emoji}  📊 {vol:>12,.0f}")
        ...
        for i, stock in enumerate(sorted_by_change[-5:], 1):
            ...
            oi_pattern = self.analyze_oi_pattern(name, price, oi)
            pattern_emoji = "🔵" if "Long Build" in str(oi_pattern) else "🟡" if "Short Cover" in str(oi_pattern) else "🔴" if "Short Build" in str(oi_pattern) else "⚪"
            print(f"  {i}. {name:20} ₹{price:>8.2f}  {Fore.RED}{change:>+6.2f}%{Style.RESET_ALL}  {pattern_emoji}  📊 {vol:>12,.0f}")

        # Momentum leaders (NEW)
        print(f"\n{Fore.CYAN}🚀 MOMENTUM LEADERS:{Style.RESET_ALL}")
        scored_stocks = []
        for stock in stocks[:20]:  # Limit for performance
            score = self.calculate_momentum_score(stock)
            if score > 0.3:  # Only show strong momentum
                scored_stocks.append((stock, score))
        
        for stock, score in sorted(scored_stocks, key=lambda x: x[1], reverse=True)[:5]:
            name = stock.get("name", "N/A")[:20]
            price = stock.get("price", 0)
            change = stock.get("pricePerChange", 0)
            print(f"  {name:20} ₹{price:>8.2f}  {Fore.GREEN}{change:>+6.2f}%{Style.RESET_ALL}  ⭐ {score:>6.2f}")
        ...
        if penny_stocks:
            ...
            for i, stock in enumerate(penny_stocks[:10], 1):
                ...
                print(f"  {i}. {stock['name']:20} ₹{stock['price']:>6.2f}  {change_color}{stock['change']:>+5.2f}%{Style.RESET_ALL}  📊 {stock['volume']:>12,.0f}")
        ...
        while self.running:
            try:
                ...
                for stock in stocks:
                    ...
                    # STORE BEFORE CHECKING - but correctly
                    # Price check uses previous value
                    price_alert = self.check_price_movement(name, price)
                    
                    # Then append current values
                    ...
                    # Volume spike check (uses deltas)
                ...
                if iteration % 3 == 0 or alerts_found or penny_stocks:  # Update display every 3 iterations
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Enhanced Real-time Stock Monitor')
    ...
    print(f"{Fore.CYAN}{'='*80}{Style.RESET_ALL}")
    print(f"{Fore.GREEN}🚀 ENHANCED REAL-TIME STOCK MONITOR{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'='*80}{Style.RESET_ALL}")
