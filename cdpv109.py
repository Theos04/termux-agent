#!/usr/bin/env python3
"""
Chrome Session Manager - Production v7
Fixed: Simple display detection without xdpyinfo dependency
"""
...
# ============================================================================
# Display Manager - Fixed: Simple socket check only
# ============================================================================
...
class DisplayManager:
    ...
    def _check_x_socket(self, display: str) -> bool:
        """Check if X socket exists - this is the only reliable check"""
        display_num = display.replace(':', '').replace('/', '')
        
        # Check Unix socket (most common in Termux with VNC)
        unix_socket = f"/tmp/.X11-unix/X{display_num}"
        if os.path.exists(unix_socket):
            logger.info(f"✓ Unix socket found: {unix_socket}")
            return True
        
        # Check TCP socket (legacy)
        try:
    ...
    def get_display(self) -> Tuple[Optional[str], Optional[int]]:
        """Get a working display - simple socket check only"""
        ...
        for session in vnc_sessions:
            ...
            if self._check_x_socket(display):
                ...
                logger.warning(f"VNC session {display} exists but X socket not found - VNC may be dead")
    ...
    def _launch_once(self, session: Dict) -> Tuple[bool, Optional[int], str]:
        ...
        if display:
            ...
            if self.display_manager._check_x_socket(display):
                ...
                logger.warning(f"Display {display} socket missing, falling back to headless")
                console.print(f"[yellow]⚠️ Display socket missing, using headless[/yellow]")
...
╔══════════════════════════════════════════════════════════════╗
║           🌐 Chrome Session Manager - Production v7        ║
║        Simple Display Detection | GUI Ready               ║
╚══════════════════════════════════════════════════════════════╝
            """
