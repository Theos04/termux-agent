...
def show_all_links(explorer: DOMExplorer):
    """Show all links with detailed information"""
    console.print()
    ...
    # Show all links grouped by type
    console.print("\n[cyan]📌 ALL INTERNAL LINKS:[/cyan]")
    if link_map['internal_links']:
        internal_table = Table(box=box.SIMPLE)
        internal_table.add_column("#", style="dim", width=4)
        internal_table.add_column("Text", style="white")
        internal_table.add_column("URL", style="cyan")
        internal_table.add_column("Target", style="dim")
        
        for i, link in enumerate(link_map['internal_links'][:20], 1):
        ...
        if len(link_map['internal_links']) > 20:
            console.print(f"[dim]... and {len(link_map['internal_links']) - 20} more internal links[/dim]")
    else:
    ...
    if link_map['external_links']:
        ...
        for domain, domain_links in sorted(domain_groups.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            for link in domain_links[:5]:
                console.print(f"   • {link.get('text', '')[:40]} → [cyan]{link.get('href', '')[:50]}[/cyan]")
            if len(domain_links) > 5:
                console.print(f"   [dim]... and {len(domain_links) - 5} more from this domain[/dim]")
    else:
        console.print("[dim]No external links found[/dim]")
    
    # Show competition links specifically
    console.print("\n[yellow]🎯 COMPETITOR LINKS:[/yellow]")
    if link_map['competition_links']:
        comp_table = Table(box=box.SIMPLE, border_style="red")
        comp_table.add_column("#", style="dim", width=4)
        comp_table.add_column("Text", style="white")
        comp_table.add_column("URL", style="cyan")
        comp_table.add_column("Domain", style="red")
    ...
    if link_map['internal_links'] and Confirm.ask("Navigate to an internal link?"):
        ...
        if len(link_map['internal_links']) > 20:
            console.print(f"  [dim]... and {len(link_map['internal_links']) - 20} more[/dim]")
