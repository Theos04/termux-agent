...
class CompetitionScout:
    def __init__(self, page: ChromePage):
        self.page = page
        self.competitions = []
        # Registration fee patterns - avoid matching prize amounts
        self.fee_patterns = [
            r'registration\s*fee[:\s]*[₹Rs. ]*([\d,]+)',
            r'entry\s*fee[:\s]*[₹Rs. ]*([\d,]+)',
            r'fee[:\s]*[₹Rs. ]*([\d,]+)',
            r'cost[:\s]*[₹Rs. ]*([\d,]+)',
            r'pay[:\s]*[₹Rs. ]*([\d,]+)'
        ]
        self.free_keywords = ['free', 'no fee', 'no cost', 'free entry', '₹0', 'Rs. 0', 'free registration', 'free of cost']
        self.prize_patterns = [
            r'prize\s*(?:money|pool|amount)?[:\s]*[₹Rs. ]*([\d,]+)',
            r'win\s*(?:up\s*to)?[:\s]*[₹Rs. ]*([\d,]+)',
            r'reward[:\s]*[₹Rs. ]*([\d,]+)',
            r'cash\s*(?:prize)?[:\s]*[₹Rs. ]*([\d,]+)',
            r'incentive[:\s]*[₹Rs. ]*([\d,]+)',
            r'grand\s*prize[:\s]*[₹Rs. ]*([\d,]+)',
            r'1st\s*prize[:\s]*[₹Rs. ]*([\d,]+)',
            r'winner\s*(?:gets|receives)[:\s]*[₹Rs. ]*([\d,]+)'
        ]
        self.internship_keywords = ['internship', 'ppo', 'pre-placement', 'placement', 'job offer', 'hiring']
        self.certificate_keywords = ['certificate', 'certified', 'certification']
    ...
    def extract_competition_details(self, text: str) -> Dict:
        ...
        reasons_set = set()  # Use set to avoid duplicates
        ...
        for keyword in self.free_keywords:
            if keyword in text_lower:
                ...
                reasons_set.add("✅ Free registration")
                break
        
        # Check for registration fee - only if not free
        if not is_free:
            for pattern in self.fee_patterns:
                fee_match = re.search(pattern, text_lower)
                if fee_match:
                    ...
                    if amount.isdigit() and int(amount) > 0:
                        ...
                        if int(amount) > 500:
                            reasons_set.add(f"💰 Registration fee: ₹{int(amount):,} (consider if worth it)")
                        elif int(amount) > 0:
                            reasons_set.add(f"💰 Registration fee: ₹{int(amount):,}")
                        break
        
        # Check for prize money
        for pattern in self.prize_patterns:
            prize_match = re.search(pattern, text_lower)
            if prize_match:
                amount = prize_match.group(1).replace(',', '')
                if amount.isdigit():
                    prize_amount = int(amount)
                    # Ignore very small amounts (likely not real prizes)
                    if prize_amount > 100:
                        details['prize_money'] = prize_amount
                        if prize_amount >= 100000:
                            reasons_set.add(f"🏆 GRAND PRIZE: ₹{prize_amount:,}")
                            details['worth_score'] += 5
                        elif prize_amount >= 50000:
                            reasons_set.add(f"🏆 Prize: ₹{prize_amount:,}")
                            details['worth_score'] += 4
                        elif prize_amount >= 10000:
                            reasons_set.add(f"💰 Prize: ₹{prize_amount:,}")
        ...
        for keyword in self.internship_keywords:
            if keyword in text_lower:
                ...
                reasons_set.add("🚀 Internship/PPO opportunity")
                details['worth_score'] += 3
                break
        ...
        for keyword in self.certificate_keywords:
            if keyword in text_lower:
                ...
                reasons_set.add("📜 Certificate provided")
                details['worth_score'] += 1
                break
        ...
        if team_match:
            ...
            reasons_set.add(f"👥 Team size: {details['team_size']}")
        ...
        if deadline_match:
            ...
            reasons_set.add(f"⏰ Deadline: {details['deadline']}")
        
        # Check for featured/prestige indicators
        if 'featured' in text_lower:
            reasons_set.add("⭐ Featured competition")
            details['worth_score'] += 1
        
        if 'top' in text_lower and 'company' in text_lower:
            reasons_set.add("🏆 Top company participation")
            details['worth_score'] += 1
        
        # Check for free certification
        if 'free certificate' in text_lower:
            reasons_set.add("🎓 Free certificate")
            details['worth_score'] += 1
        
        # Bonus: Check if it's free and has good prize
        if details['is_free'] and details['prize_money'] and details['prize_money'] > 5000:
            reasons_set.add("💎 FREE entry + Great prize!")
            details['worth_score'] += 2
        
        # Convert set to list
        details['reasons_to_apply'] = list(reasons_set)
    ...
    def get_worth_status(self, details: Dict) -> tuple:
        ...
        prize = details.get('prize_money', 0) or 0
        has_internship = details.get('has_internship', False)
        
        # Boost score if free with good benefits
        if is_free and (prize > 5000 or has_internship):
            score += 3
            return "🔥 HOT 🔥", "red", score
        elif is_free and (prize > 0 or has_internship):
            score += 2
            return "🌟 EXCELLENT", "green", score
        elif is_free:
            return "✅ WORTH IT", "yellow", score
        elif score >= 5:
...
class CompetitionScoutApp:
    ...
    def find_competition_links(self):
        ...
        for link in links:
            href = link.get('href', '')
            if href and self.scout.is_competition_page(href):
        ...
        if competition_links:
            console.print(f"[green]✅ Found {len(competition_links)} unique competition links[/green]")
    ...
    def view_results(self):
        ...
        with_prize = sum(1 for c in self.analyzed_competitions if c.get('prize', 0) is not None and c.get('prize', 0) > 0)
        ...
        for i, comp in enumerate(self.analyzed_competitions[:30], 1):
            ...
            fee = "✅ FREE" if comp.get('is_free', False) else f"₹{comp.get('fee', '?')}" if comp.get('fee') else "—"
            prize = f"₹{comp['prize']:,}" if comp.get('prize') and comp.get('prize') > 0 else "—"
        ...
        if top_comps:
            for i, comp in enumerate(top_comps, 1):
                ...
                if comp.get('reasons'):
                    for reason in comp.get('reasons', [])[:4]:
    ...
    def save_results(self):
        ...
        categorized = {
            ...
            'excellent': [c for c in self.analyzed_competitions if 'EXCELLENT' in c.get('status', '')],
            ...
            for category, comps in categorized.items():
                if comps:
                    ...
                    for comp in comps:
                        ...
                        f.write(f"Free: {'YES' if comp.get('is_free', False) else 'NO'}\n")
                        f.write(f"Prize: {comp.get('prize', 'N/A')}\n")
                        f.write(f"Internship: {'YES' if comp.get('has_internship', False) else 'NO'}\n")
                        if comp.get('reasons'):
                            f.write("Why apply:\n")
