...
def extract_hackathon_urls():
    ...
    function extractHackathonUrls() {
        ...
        // Also check for hackathon cards
...
def main():
    print("\n" + "=" * 60)
    print("📊 UNSTOP HACKATHON URL MAPPER")
    ...
    for page_url in pages:
        ...
        hackathon_urls = extract_hackathon_urls()
        
        # Get all links for debugging
        all_links = get_all_links()
        all_links_data[page_url] = all_links
        
        if hackathon_urls:
            print(f"✅ Found {len(hackathon_urls)} hackathon URLs")
            all_hackathon_urls.extend(hackathon_urls)
            
            # Show sample
            print(f"  Sample URLs:")
            for url in hackathon_urls[:3]:
                print(f"    • {url}")
        else:
            print(f"⚠️ No hackathon URLs found on this page")
    ...
    # Deduplicate URLs
    all_hackathon_urls = list(dict.fromkeys(all_hackathon_urls))
    
    if all_hackathon_urls:
        print(f"\n📊 Total unique hackathon URLs found: {len(all_hackathon_urls)}")
        
        # Save to file
        with open('hackathon_urls.txt', 'w', encoding='utf-8') as f:
            f.write('\n'.join(all_hackathon_urls))
        ...
        results = {
            'timestamp': time.time(),
            'total_hackathons': len(all_hackathon_urls),
            'hackathon_urls': all_hackathon_urls,
            'pages_processed': pages,
            'all_links': all_links_data
        }
        
        with open('hackathon_urls.json', 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2)
        
        print("💾 Saved to: hackathon_urls.json")
        
        # Count by status
        open_count = sum(1 for u in all_hackathon_urls if 'oppstatus=open' in u)
        closed_count = sum(1 for u in all_hackathon_urls if 'oppstatus=closed' in u)
        upcoming_count = sum(1 for u in all_hackathon_urls if 'oppstatus=upcoming' in u)
        ...
        # Show all URLs
        print(f"\n📋 All URLs:")
        for i, url in enumerate(all_hackathon_urls, 1):
            print(f"  {i}. {url}")
    else:
